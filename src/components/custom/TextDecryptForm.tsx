...
function TextDecryptForm({ formType, inputLabel, submitCallback }: TextDecryptFormProps) {
    const { textEncrypted } = useTextCipher();
    const textDecryptForm = z.object({
        input: z.string().min(1),
        c1_prime: z.coerce.number().min(-1).max(1),
        c2_prime: z.coerce.number().min(-1).max(1),
    })

    const form = useForm<z.infer<typeof textDecryptForm>>({
        resolver: zodResolver(textDecryptForm),
        defaultValues: {
    ...
    const onSubmit = async (data: z.infer<typeof textDecryptForm>) => {
        console.log("Submit from TextDecryptForm", data);
...
export default TextDecryptForm;